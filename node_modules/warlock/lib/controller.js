/*
* remove the render and layout method, I just need control funtions
*/
var cache = {}, fs = require('fs'),
    path        = require('path'),
    // import warlock utils
    utils       = require('./utils'),
...
/**
 * Internal request handler. Serves request using happ env:
 *
...
Controller.getPathTo = function () {
    return happ.routeMapper.pathTo;
};

/**
 * Add custom base controller dir to happ pool. It allows you to build
 ...
 *
 * @public happ.controller.addBasePath
 */
function addBasePath(basePath, prefix, context, root) {
    prefix = prefix || '';
    if (!utils.existsSync(basePath)) return;
...
Controller.prototype.protectFromForgery = function protectFromForgery(secret, paramName) {
    ...
    req.csrfParam = paramName || 'token';
    req.csrfToken = sign(req.session.csrfToken);

    if (req.originalMethod == 'POST') {
        var token = req.param('token');
        if (!token || token !== sign(req.session.csrfToken)) {
            happ.logger.write('Incorrect authenticity token');
            this.send(403);
        } else {
            this.next();
        }
